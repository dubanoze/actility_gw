...
#define SPI_CSN_DEV_PATH	"/dev/p1_20"

/* -------------------------------------------------------------------------- */
/* --- PUBLIC FUNCTIONS DEFINITION ------------------------------------------ */

/* SPI initialization and configuration */
int lgw_fpga_spi_open(void **spi_target_ptr, void **spi_csn_target_ptr) {
	int *spi_device = NULL;
	int *spi_csn_device = NULL;
	int dev;
	int dev_csn;
	int a=0, b=0;
	int i;
	...
	spi_csn_device = malloc(sizeof(int));
	if (spi_csn_device == NULL) {
		DEBUG_MSG("ERROR: MALLOC FAIL\n");
		return LGW_SPI_ERROR;
	}
	...
	dev_csn = open(SPI_CSN_DEV_PATH, O_RDWR|O_NONBLOCK);
	if (dev_csn < 0) {
		DEBUG_PRINTF("ERROR: failed to open SPI device %s\n", SPI_CSN_DEV_PATH);
		return LGW_SPI_ERROR;
	}
	...
	/* set FPGA_CSN default value to high level*/
    i = ioctl(dev_csn, LPC32X0_GPIO_WRITE_HIGH_LEVEL, NULL);
	
	*spi_device = dev;
	*spi_csn_device = dev_csn;
	*spi_target_ptr = (void *)spi_device;
	*spi_csn_target_ptr = (void *)spi_csn_device;
...
/* SPI release */
int lgw_fpga_spi_close(void *spi_target, void *spi_csn_target) {
	int spi_device;
	int spi_csn_device;
	int a;
	int b;
...
/* Simple write */
int lgw_fpga_spi_w(void *spi_target, void *spi_csn_target, uint8_t address, uint8_t data) {
	int spi_device;
	int spi_csn_device;
	uint8_t out_buf[2];
	...
	k.cs_change = 1;
	k.bits_per_word = 8;
	b = ioctl(spi_csn_device, LPC32X0_GPIO_WRITE_LOW_LEVEL, NULL); /* Set HOST_CSN_FPGA Low */
	a = ioctl(spi_device, SPI_IOC_MESSAGE(1), &k);
	b = ioctl(spi_csn_device, LPC32X0_GPIO_WRITE_HIGH_LEVEL, NULL); /* Set HOST_CSN_FPGA High */
...
/* Simple read */
int lgw_fpga_spi_r(void *spi_target, void *spi_csn_target, uint8_t address, uint8_t *data) {
	int spi_device;
	int spi_csn_device;
	uint8_t out_buf[2];
...
int lgw_fpga_spi_wb(void *spi_target, void *spi_csn_target, uint8_t address, uint8_t *data, uint16_t size) {
	int spi_device;
	int spi_csn_device;
	uint8_t command;
	...
	for (i=0; size_to_do > 0; ++i) {
		...
		k[1].len = chunk_size;
		j = ioctl(spi_csn_device, LPC32X0_GPIO_WRITE_LOW_LEVEL, NULL); /* Set HOST_CSN_FPGA Low */
		byte_transfered += (ioctl(spi_device, SPI_IOC_MESSAGE(2), &k) - 1 );
		j = ioctl(spi_csn_device, LPC32X0_GPIO_WRITE_HIGH_LEVEL, NULL); /* Set HOST_CSN_FPGA High */
...
int lgw_fpga_spi_rb(void *spi_target, void *spi_csn_target, uint8_t address, uint8_t *data, uint16_t size) {
	int spi_device;
	int spi_csn_device;
	uint8_t command;
	struct spi_ioc_transfer k[2];
