...
cat $DEST_DIRECTORY/lrr/opkg/CONTROL_wirmana/control | sed "s/_REPLACE_VERSION_/$VERSION/" > ${OPKG_DIR}/CONTROL/control

cp $DEST_DIRECTORY/lrr/opkg/CONTROL_wirmana/postinst ${OPKG_DIR}/CONTROL/postinst

$ROOTACT/opkg-build -o root -g root ${OPKG_DIR} $ROOTACT/deliveries

rm -rf $OPKG_DIR
# remove tar.gz file to be sure only .ipk can be delivered
rm $LRRFILE
md5file=${LRRFILE%.tar.gz}.md5
rm $md5file
