...
#include "loragw_hal.h"		// Specific to FCMLB
#endif
...
#if defined(FCMLB) || defined(FCPICO) || defined(FCLAMP)
	/* Brian, Add delay for AGC MCU reset */
	wait_ms(5);
#endif
...
void usage(void) {
	...
	printf( " -n number of RSSI captures, each capture is 4095 samples long, default is 90 (3s for 125Khz capture rate)\n");
...
int main(int argc, char **argv)
{
	...
	int capture_wait;
#if defined(FCMLB) || defined(FCPICO) || defined(FCLAMP)
	char *spidevice = NULL;	// Specific to FCMLB
#endif
	
	/* parse command line options */
#if defined(FCMLB) || defined(FCPICO) || defined(FCLAMP)
	while ((i = getopt_long (argc, argv, "d:h:n:p:", long_options, &option_index)) != -1) {
#else
	while ((i = getopt_long (argc, argv, "h:n:p:", long_options, &option_index)) != -1) {
#endif
		switch (i) {
			case 0:
...
#ifdef OLD_LGW_CONNECT_PROTOTYPE
        reg_stat = lgw_connect();
#else
        reg_stat = lgw_connect(false, DEFAULT_TX_NOTCH_FREQ);
#endif
		...
		for (k=0; k<nb_captures; k++)
		{
			...
			for (i=0; i<64; i++) {
				...
				for (j=0; j<64; j++) {
                    ...
                    if ((i == 0) && (j == 0 )) {
                        /* The first sample of the capture is incorrect, we drop it */
                        continue;
                    }
                    rssi_hist[(int)rssi]++;
				}
