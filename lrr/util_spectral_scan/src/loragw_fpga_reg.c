...
int lgw_fpga_connect(void) {
	...
	if (spi_target != NULL) {
		...
		lgw_fpga_spi_close(spi_target, spi_csn_target);
	}
	
	/* open the SPI link */
	spi_stat = lgw_fpga_spi_open(&spi_target, &spi_csn_target);
	...
	spi_stat = lgw_fpga_spi_w(spi_target, spi_csn_target, fpga_regs[LGW_FPGA_SPI_MUX_CTRL].addr, FPGA_SPI_MUX_REG);
	spi_stat |= lgw_fpga_spi_r(spi_target, spi_csn_target, fpga_regs[LGW_FPGA_VERSION].addr, &u);
	if (spi_stat != LGW_SPI_SUCCESS) {
...
int lgw_fpga_reg_w(uint16_t register_id, int32_t reg_value) {
	...
	if ((r.leng == 8) && (r.offs == 0)) {
		/* direct write */
		spi_stat += lgw_fpga_spi_w(spi_target, spi_csn_target, r.addr, (uint8_t)reg_value);
	} else if ((r.offs + r.leng) <= 8) {
		/* single-byte read-modify-write, offs:[0-7], leng:[1-7] */
		spi_stat += lgw_fpga_spi_r(spi_target, spi_csn_target, r.addr, &buf[0]);
		...
		spi_stat += lgw_fpga_spi_w(spi_target, spi_csn_target, r.addr, buf[3]);
		...
		spi_stat += lgw_fpga_spi_wb(spi_target, spi_csn_target, r.addr, buf, size_byte); /* write the register in one burst */
	} else {
...
int lgw_fpga_reg_r(uint16_t register_id, int32_t *reg_value) {
	...
	if ((r.offs + r.leng) <= 8) {
		...
		spi_stat += lgw_fpga_spi_r(spi_target, spi_csn_target, r.addr, &bufu[0]);
		...
		size_byte = (r.leng + 7) / 8; /* add a byte if it's not an exact multiple of 8 */ 
		spi_stat += lgw_fpga_spi_rb(spi_target, spi_csn_target, r.addr, bufu, size_byte);
		u = 0;
...
int lgw_fpga_reg_wb(uint16_t register_id, uint8_t *data, uint16_t size) {
	...
	/* do the burst write */
	spi_stat += lgw_fpga_spi_wb(spi_target, spi_csn_target, r.addr, data, size);
...
int lgw_fpga_reg_rb(uint16_t register_id, uint8_t *data, uint16_t size) {
	...
	/* do the burst read */
	spi_stat += lgw_fpga_spi_rb(spi_target, spi_csn_target, r.addr, data, size);
...
int lgw_sx1272_reg_w(uint8_t address, uint8_t reg_value) {
	return lgw_fpga_spi_w(spi_target, spi_csn_target, address, reg_value);
}

/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */

int lgw_sx1272_reg_r(uint8_t address, uint8_t *reg_value) {
	return lgw_fpga_spi_r(spi_target, spi_csn_target, address, reg_value);
}
